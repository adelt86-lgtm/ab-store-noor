  ...
  return (
    <main dir="rtl" className="ab-dashboard">
      <aside className="ab-sidebar">
        <div className="ab-logo"><span className="ab-logo-mark">N</span><div><b>النور</b><small>Store Control</small></div></div>
      ...
      <section className="ab-main">
        ...
        {tab === "channels" && <ChannelsPanel settings={settings} setSettings={setSettings} />}
...
function ChannelsPanel({settings,setSettings}:{settings:StoreSettings,setSettings:Dispatch<SetStateAction<StoreSettings>>}){const channels=[['واتساب','طلبات ورسائل العملاء',true,MessageCircle],['Facebook','صفحة المتجر والتفاعل',false,Users],['Instagram','الرسائل والمحتوى',false,Smartphone],['Messenger','محادثات الصفحة',false,MessageCircle],['Telegram','طلبات ومحادثات',false,Smartphone]] as const;return <div className="ab-content"><div className="panel large"><div className="panel-head"><div><span className="ab-kicker">CHANNELS</span><h3>قنوات التواصل</h3><p>واجهة موحدة لتفعيل القنوات وربطها لاحقاً بالـ API.</p></div></div><div className="channels-list">{channels.map(([name,desc,on,Icon])=><div className="channel-row" key={name}><div className="channel-icon"><Icon size={19}/></div><div><b>{name}</b><small>{desc}</small></div><span className={`channel-status ${on?'on':''}`}>{on?'مفعّل':'جاهز للربط'}</span><button className="switch" aria-label={`تفعيل ${name}`}><i/></button></div>)}</div><div className="info-box"><Zap size={18}/><div><b>التحكم الحقيقي</b><p>هذه الصفحة مجهزة كطبقة تحكم للواجهة. ربط الحالة مع قاعدة البيانات وواجهات Meta/WhatsApp يجعل التغييرات مشتركة بين جميع الأجهزة.</p></div></div><div className="form-grid one"><Field label="رقم واتساب الحالي" value={settings.whatsapp} onChange={v=>setSettings(s=>({...s,whatsapp:v}))}/></div></div></div>}
